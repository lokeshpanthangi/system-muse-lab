...
import { Save, Send, Eye, ChevronLeft, ChevronRight, Clock, Trash2, Download, X } from "lucide-react";
...
export default function Practice() {
  ...
  useEffect(() => {
    ...
    if (savedDraft) {
      try {
        ...
        if (draft.diagramData && excalidrawAPI) {
          excalidrawAPI.updateScene(draft.diagramData);
        }
  ...
  }, [id, excalidrawAPI, notes, elapsedTime]);
  ...
  const handleExportImage = () => {
    if (excalidrawAPI) {
      ...
      excalidrawAPI.exportToBlob({ mimeType: "image/png" }).then((blob: Blob) => {
